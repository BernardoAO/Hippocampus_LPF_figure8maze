...
for animal = animal_numbers
    ...
    %try
    [vel_grid, ratio_grid, tran, ratio_vel] = ...
        theta_delta_ratio(pideals, lfps, opt_td);

    results(a).vel_grid = vel_grid;
    results(a).ratio_grid = ratio_grid;
    results(a).norm_tran_down = tran.down{2};
    results(a).norm_tran_up = tran.up{2};
...
%%
plot_mean_grid(results,"ratio_grid", "theta power", opt_td)
for g = 1:2
    r = results([results.genotype] == g);
    plot_mean_grid(r,"ratio_grid", "theta power " + genotypes(g), opt_td)
end
%% Results
