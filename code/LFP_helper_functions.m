...
function conv_result = conv_LFP(lfp, fs, freq_band, n_cycles)
    % Obtains the power of a band via the convolution with a morlet
    % wavelett.
    %
    % Inputs:
...
function power = power_LFP_multitaper(lfp, fs, freq_band, TW, K)
    % Obtains the power of an LFP signal via the multitaper method.
    %
    % Inputs:
    % lfp: 1d array with the LFP signal
    % fs: sampling time_vec
...
function [power_sm, phase] = LFP_Hilbert(lfp, d, opt)
    % Obtains the power and phase of an LFP signal via a Hilbert transfrom.
    %
    % Inputs:
    % lfp: 1d array with the LFP signal
    % b, a: butter filter 
...
function power_pt = get_power(pideal, lfp, opt)
    % Given a pideal object, obtains the power of an LFP signal for each
    % element in pideal.
    %
    % Inputs:
...
function tran = get_transitions(x, y, y_bound, win)
    % Obtains the start of each lap in the fig8maze given a y boundary. 
    %
    % Inputs:
    % x, y : position
    ...
    % Outputs:
    % tran : matrix shape (3,n_tran) (time index, left/right, hit/miss)
    tran_ind = [];
    tran_typ = [];
    hits = 1;
...
function p_ideal = get_p_ideal(pos, std_grid, d_lim)
    % Calculates the idealized path given a standarized grid.
    %
    % Inputs:
    % pos : [x; y], shape (2,t)
    % std_grid : [x; y], shape (2,t)
    % d_lim : max travel distance in a video frame
    % Outputs:
    % p_ideal : shape (2,t)
...
function std_grid = make_std_grid(range, n_bin)
    % Makes a standard grid for the figure 8 maze
    %
    % Inputs:
    % range : [x_min,x_max; y_min,y_max]
    % n_bin : [n_x, n_y]
    % Outputs:
    % std_grid : shape (2,n_points)
...
function plot_day(std_grid, pos, tran, animal, iii, sp, block_c)
    % plots the x and y position with the standard grid background, and a
    % raster indicating hit or miss with green/red.
...
function plot_performance(results, colors, sp, plot_scatter)
    % plots the percentage of hits for a set of mice for each session.
...
function plot_mean_grid(results, gname, name, opt)
    % Plots the mean grid (spatial visualization of a metric) for a set of 
    % mice.
...
function plot_ratio_velocity(results, colors, genotypes, opt)
    % Plots the mean and std for the velocity
